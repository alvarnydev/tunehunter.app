...
const ProfileButton = ({ menuOpen, setMenuOpen }: { menuOpen: boolean; setMenuOpen: React.Dispatch<React.SetStateAction<boolean>> }) => {
  ...
  return (
    <div className='relative z-30 flex justify-center items-center h-full'>
      <button className={`btn btn-active border-none rounded-full px-3 md:px-4 ${isAuthenticated ? 'btn-primary hover:bg-primary ' : 'btn-ghost no-animation cursor-not-allowed'}`} onClick={handleProfileClick}>
